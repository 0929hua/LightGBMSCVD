...
dataMat= np.loadtxt('D:/pyProjects/pythonProject/second/word2vec/dao/labelListDAOs.txt',dtype=np.float64)
print(dataMat)
...
with open("dao/daoPCA.txt", "w") as fp:
    fp.writelines("\n".join([" ".join([str(i) for i in j]) for j in lowDDataMat]))
fp.close()

# x=0
# textl = open("dao/daoPCA.txt", "w")
# while x <2084:
#     textl.write('\n')
#     m=str(lowDDataMat[x]).strip('[')
#     m = m.strip(']')
#     m=m.split(' ')
#     for i in m:
#         if i.__eq__(''):
